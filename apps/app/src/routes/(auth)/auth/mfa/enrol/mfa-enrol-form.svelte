<script lang="ts">
	...
	const { form: _form, challenge }: Pick<PageData, 'form' | 'challenge'> = $props();

	let formDisable = $state(false);

	const form = superForm(_form, {
		validators: zodClient(mfaEnrollmentFormSchema),
		onSubmit: () => {
			formDisable = true;
		},

		onResult: ({ result }) => {
			if (!['redirect'].includes(result.type)) {
				formDisable = false;
			}
		}
	});
...
<form method="POST" use:enhance>
	...
	<Form.Button class="mt-4 w-full" disabled={formDisable}>Enrol</Form.Button>
</form>
