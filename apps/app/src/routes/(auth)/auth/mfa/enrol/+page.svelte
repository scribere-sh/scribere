<script lang="ts">
	...
	const { challenge, form, encrollmentSVG, enrolmentUrl } = data;
</script>
...
<Card.Root class="w-96">
	<Card.Header>
		...
		<Card.Description
			>Scan the QRCode or click the button to copy enrolment code to enrol in 2FA</Card.Description
		>
	</Card.Header>
	<Card.Content class="flex flex-col items-center gap-2">
		<div class="mx-auto mt-4 aspect-square w-full rounded-sm bg-white p-4">
			{@html encrollmentSVG}
		</div>

		<Button disabled={buttonDisabled} onclick={copyData}>Copy Code</Button>
