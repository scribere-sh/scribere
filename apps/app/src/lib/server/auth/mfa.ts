...
import { createArgon2id, decodeTokenStringToBytes, generateTokenString } from './cryptography';
...
export const verifyOTP = (key: string, digits: string) => {
    ...
    return verifyTOTPWithGracePeriod(keyBytes, MFA_PERIOD_SECONDS, MFA_DIGITS, digits, MFA_GRACE_PERIOD_SECONDS);
}

export const enrolUserWithTOTP = async (userId: string, key: string, initial: string): Promise<boolean> => {
    if (!verifyOTP(key, initial)) {
        return false;
    }

    await DB
        .insert(twoFactorAuthenticationProviderTable)
        .values({
            userId,
            type: TOTP_TYPE,
            challenge: key
...
export const verifyUserOTP = async (userId: string, digits: string): Promise<boolean> => {
    const [otpChallengeLookup] = await DB
        .select({challenge: twoFactorAuthenticationProviderTable.challenge})
        .from(twoFactorAuthenticationProviderTable)
        .where(and(
            eq(twoFactorAuthenticationProviderTable.userId, userId),
            eq(twoFactorAuthenticationProviderTable.type, TOTP_TYPE)
